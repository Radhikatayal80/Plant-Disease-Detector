...
def load_model(path):
    ...
    # Xception Model
    xception_model = tf.keras.models.Sequential(name='xception_model')
    xception_model.add(tf.keras.applications.xception.Xception(include_top=False, weights='imagenet', input_shape=(512, 512, 3)))
    xception_model.add(tf.keras.layers.GlobalAveragePooling2D())
    xception_model.add(tf.keras.layers.Dense(4, activation='softmax'))

    # DenseNet Model
    densenet_model = tf.keras.models.Sequential(name='densenet_model')
    densenet_model.add(tf.keras.applications.densenet.DenseNet121(include_top=False, weights='imagenet', input_shape=(512, 512, 3)))
...
if uploaded_file is not None:
    ...
    st.image(np.array(image.resize((700, 400), Image.BICUBIC)), width=None)
